...
static void ReduceTextures()
{
	...
	bool *Used = (bool *)calloc (l->nummiptex, sizeof(bool));
	hlassume (Used != NULL, assume_NoMemory);

	int *Map = (int *)malloc (l->nummiptex * sizeof(int));
	hlassume (Map != NULL, assume_NoMemory);

	int *lumpsizes = (int *)malloc (l->nummiptex * sizeof (int));
	hlassume (lumpsizes != NULL, assume_NoMemory);

	const int newdatasizemax = g_texdatasize - ((byte *)&l->dataofs[l->nummiptex] - (byte *)l);
	...
	for (int i = 0; i < g_numtexinfo; i++)
	{
		...
		if (t->miptex < 0 || t->miptex >= l->nummiptex)
		{
